...
function makeDefaultRectangularPocket(x: number, y: number): RectangularPocket {
  const p = new RectangularPocket()
  p.pos    = { x, y, z: 0 }
  p.width  = 10
  p.height = 6
  p.depth  = 1
...
export class RectangularPocketTool implements ToolHandler {
  ...
  onPointerUp(e: PointerEvent, ctx: ToolContext): void {
    ...
    const item = makeDefaultRectangularPocket(snapped.x, snapped.y)
    ctx.pushHistory()
  ...
  drawOverlay(ctx2d: CanvasRenderingContext2D, ctx: ToolContext): void {
    ...
    const preview = makeDefaultRectangularPocket(x, y)
